...
`ngOnChanges` 方法获取了一个对象，它可以同时观测多个绑定的属性值，它把每个发生变化的属性名都映射到一个 `SimpleChange` 对象，该对象中有属性的当前值和前一个值。

触发条件：
`@Input` 属性发生变化时调用。当输入属性为对象时，对象的属性值发生变化时不会调用，当对象的引用变化时会触发

首次调用发生在 `OnInit` 之前。当然必须是父组件使子组件的输入属性变化时才会调用。
