...
- `tabindex=负值` (例如 `tabindex="-1"`)，表示元素不会被 Tab 键选中，但可以通过 JavaScript 聚焦。
- `tabindex="0"`，将元素添加到正常的 Tab 顺序中，它的相对顺序是当前处于的 DOM 结构来决定的。
- `tabindex=正值`，可以指定元素的 Tab 顺序，数值越小，越先被选中。如果多个元素拥有相同的 `tabindex`，它们的相对顺序按照他们在当前 DOM 中的先后顺序决定。`tabindex=0` 的顺序会被放到最后。 例如：`1,1,2,2,2,2,3,4,5,0,0,0 ...`。`tabindex` 的值是**非法值**或者**没有设置 `tabindex`**的元素，也是放到最后。
